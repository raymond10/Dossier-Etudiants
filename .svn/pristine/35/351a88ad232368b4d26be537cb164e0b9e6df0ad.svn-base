...
public class EditionCtrl extends AComponentCtrl {
	...
	public WOActionResults mySearchPv() throws Throwable {
		...
		try {
			...
			if (!type.equals("4") && (diplome == null || diplome.equals("")))
				throw new Exception(
						"Le nombre de données à remonter est important, merci d'affiner vos options d'impression (inclure un diplôme par exemple)");
			if (wocomponent.getNumero() != null && !wocomponent.getNumero().equals("")) {
				...
				if (!type.equals("4") && (niveau == null || niveau.equals("")))
					throw new Exception(
							"Le nombre de données à remonter est important, merci d'affiner vos options d'impression (inclure un niveau par exemple)");
			}

			print = "Les éditions sont en cours... elles seront disponibles d'ici quelques instants dans votre boîte mail.";
			char c = '-';
				...
				if (types.contains(type)) {
					...
					if (((periode != null && periode.contains("U")) && (diplome != null && diplome
							...
							|| ((periode != null && periode.contains("U")) && (diplome != null && diplome
									.contains("MST")))) {
						searchError = "Il n'y a pas d'étudiant correspondant à ces critères de recherche. ";
						printMst = null;
						print = null;
						printError = null;
					} else {
						...
						try {
							...
							if (urlResponseView.contains("Exception")) {
								printError = " Les éditions ne peuvent pas être lancées. Essayer de sélectionner un niveau sinon contacter le support.";
								sess.addSimpleErrorMessage(
										"Editions",
										"Les éditions ne peuvent pas être lancées. Essayer de sélectionner un niveau sinon contacter le support.");
								searchError = null;
								printMst = null;
								print = null;
							}
	...
	public WOActionResults mySearchMst() throws Throwable {
		...
		try {
			...
			printMst = "Les éditions sont en cours... elles seront disponibles d'ici quelques instants dans votre boîte mail.";
			if (reoriente == null) {
				reoriente = "O";
			}
				...
				if (action.equals(wocomponent.action)) {
					...
					if ((periode != null && periode.contains("U"))
						...
						try {
							...
							// TODO Auto-generated catch block
							e.printStackTrace();
							printError = "Les éditions ne peuvent pas être lancées. Contactez le support.";
							sess.addSimpleErrorMessage("Editions",
									"Les éditions ne peuvent pas être lancées. Contactez le support.");
							searchError = null;
							printMst = null;
							print = null;
						}
	...
	// Methode d'affichage des 5 derniers editions du user connecté
