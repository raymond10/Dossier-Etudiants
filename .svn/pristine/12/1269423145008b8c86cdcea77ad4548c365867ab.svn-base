...
public abstract class StringOperation {
	...
	public static final String PASAUTORISE = "pasautorise";
...
public static ERXKey<String> sansAccent(ERXKey<String> chaine) {  
    	
    	StringBuffer Result  = new StringBuffer(chaine.key());
      
       for(int bcl = 0 ; bcl < Result.length() ; bcl++) {   
    	   int carVal = chaine.key().charAt(bcl);
           if( carVal >= MIN && carVal <= MAX ) {  
        	   // Remplacement
              String newVal = (String)map.get( carVal - MIN );
              Result.replace(bcl, bcl+1,newVal);
       ...
       ERXKey<String> res = new ERXKey<String>(Result.toString());
       return res;
   }
