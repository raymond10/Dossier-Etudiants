	...
	function b(g) {
		...
		g.onKeyDown.add(function(k, o) {
			...
			if (l || o.keyCode == e) {
				...
				k.getDoc().execCommand(l ? "ForwardDelete" : "Delete", false,
						null);
				p = i.getParent(j.startContainer, i.isBlock);
				d.each(i.select("span.Apple-style-span,font.Apple-style-span",
						p), function(r) {
					var q = i.createRng();
					q.setStartBefore(r);
					q.setEndBefore(r);
	...
	m
			.create(
					"tinymce.Editor",
					{
						...
						init : function() {
							var r, H = this, I = H.settings, E, A, D = H
									.getElement(), q, p, F, y, C, G, z, v = [];
							m.add(H);
							I.aria_label = I.aria_label
									|| n.getAttrib(D, "aria-label", H
							...
							if (I.theme) {
								...
								if (H.theme.init && I.init_theme) {
									H.theme.init(H, h.urls[I.theme]
											|| m.documentBaseURL.replace(/\/$/,
													""))
								}
							}
							function B(J) {
								var K = c.get(J), t = c.urls[J]
										|| m.documentBaseURL.replace(/\/$/, ""), s;
							...
							if (I.popup_css !== false) {
								if (I.popup_css) {
									I.popup_css = H.documentBaseURI
											.toAbsolute(I.popup_css)
								} else {
									I.popup_css = H.baseURI
											.toAbsolute("themes/" + I.theme
													+ "/skins/" + I.skin
													+ "/dialog.css")
								}
							}
							if (I.popup_css_add) {
								I.popup_css += ","
										+ H.documentBaseURI
												.toAbsolute(I.popup_css_add)
							}
							H.controlManager = new m.ControlManager(H);
							if (I.custom_undo_redo) {
								H.onBeforeExecCommand.add(function(t, J, u, K,
										s) {
									if (J != "Undo" && J != "Redo"
											&& J != "mceRepaint"
											&& (!s || !s.skip_undo)) {
							...
							if (I.render_ui) {
								...
								A = I.height || D.style.height
										|| D.offsetHeight;
								H.orgDisplay = D.style.display;
								G = /^[0-9\.]+(|px)$/i;
								if (G.test("" + E)) {
									E = Math.max(parseInt(E)
											+ (q.deltaWidth || 0), 100)
								}
								if (G.test("" + A)) {
									A = Math.max(parseInt(A)
											+ (q.deltaHeight || 0), 100)
								}
								q = H.theme.renderUI( {
									targetNode : D,
							...
							if (I.content_css) {
								m.each(g(I.content_css), function(s) {
									H.contentCSS.push(H.documentBaseURI
											.toAbsolute(s))
								})
							}
							A = (q.iframeHeight || A)
							...
							H.iframeHTML += '</head><body id="' + y
									+ '" class="mceContentBody ' + C
									+ '"><br></body></html>';
							if (m.relaxedDomain
									&& (b || (m.isOpera && parseFloat(opera
											.version()) < 11))) {
								F = 'javascript:(function(){document.open();document.domain="'
										+ document.domain
						...
						setupIframe : function() {
							var q = this, v = q.settings, x = n.get(q.id), y = q
									.getDoc(), u, p;
							...
							if (!q.settings.allow_html_in_named_anchor) {
								q.parser.addAttributeFilter("name", function(s,
										t) {
							...
							q.parser.addAttributeFilter("src,href,style",
									function(s, t) {
										...
										while (z--) {
											...
											if (!B.attributes.map[A]) {
												if (t === "style") {
													B.attr(A, D.serializeStyle(
															D.parseStyle(C),
															B.name))
												} else {
													B.attr(A, q.convertURL(C,
															t, B.name))
												}
											}
										}
							...
							q.serializer = new m.dom.Serializer(v, q.dom,
									q.schema);
							q.selection = new m.dom.Selection(q.dom,
									q.getWin(), q.serializer);
							...
							q.controlManager.onPostRender.dispatch(q,
									q.controlManager);
							...
							if (v.handle_node_change_callback) {
								q.onNodeChange.add(function(t, s, z) {
									q.execCallback(
											...
											q.id, z, -1, -1, true, q.selection
													.isCollapsed())
								})
							}
							if (v.save_callback) {
								q.onSaveContent.add(function(s, z) {
									var t = q.execCallback("save_callback",
											q.id, z.content, q.getBody());
									if (t) {
										z.content = t
									}
								})
							...
							if (v.cleanup_callback) {
								...
								q.onPreProcess.add(function(s, t) {
									if (t.set) {
										q.execCallback("cleanup_callback",
							...
							if (v.handle_event_callback) {
								q.onEvent.add(function(s, t, z) {
									if (q.execCallback("handle_event_callback",
											t, s, z) === false) {
										j.cancel(t)
									}
								})
							...
							if (a) {
								...
								q.onSetContent.add(q.selection.onSetContent
										.add(r))
							}
							q.load( {
								initial : true,
							...
							q.initialized = true;
							q.onInit.dispatch(q);
							q.execCallback("setupcontent_callback", q.id, q
									.getBody(), q.getDoc());
						...
						focus : function(u) {
							var y, q = this, s = q.selection, x = q.settings.content_editable, r, p, v = q
									.getDoc();
							if (!u) {
								r = s.getRng();
								if (r.item) {
						...
						setContent : function(u, s) {
							...
							if (s.format !== "raw") {
								u = new m.html.Serializer( {}, r.schema)
										.serialize(r.parser.parse(u))
							}
							s.content = m.trim(u);
						...
						_addEvents : function() {
							...
							B.onClick.add(function(s, t) {
								...
								if (m.isWebKit && t.nodeName == "IMG") {
									B.selection.getSel().setBaseAndExtent(t, 0,
											t, 1)
								}
								if (t.nodeName == "A"
										&& q.hasClass(t, "mceItemAnchor")) {
							...
							if (m.isWebKit) {
								q.bind(B.getDoc(), "selectionchange",
										function() {
											if (B.selectionTimer) {
												clearTimeout(B.selectionTimer);
												B.selectionTimer = 0
											}
											B.selectionTimer = window
													.setTimeout(function() {
														B.nodeChanged()
													}, 50)
										})
		...
		function T(Z, ah, ac) {
			...
			function ab() {
				...
				if (aq != al && am.endOffset == 0) {
					var ap = an(aq, al);
					var ao = ap.nodeType == 3 ? ap.length
							: ap.childNodes.length;
					am.setEnd(ap, ao)
				}
				return am
			}
			...
			if (ai) {
				if (ac) {
					...
					if (!ae || !ai.inline
						...
						if (ai.styles
								&& (ai.styles.color || ai.styles.textDecoration)) {
		...
		function Q(ad, Y, ac) {
			...
			if (af()) {
				...
				if (!P.isListening && af()) {
					...
					var Z = false;
					O("onKeyDown,onKeyUp,onKeyPress,onMouseUp".split(","),
							function(ah) {
